...
class Payment extends PayPalResourceModel {
	...
	public function addTransaction($transaction) {
		if (! $this->getTransactions ()) {
			return $this->setTransactions ( array (
					$transaction 
			) );
		} else {
			return $this->setTransactions ( array_merge ( $this->getTransactions (), array (
					$transaction 
			) ) );
		}
	}
	...
	public function removeTransaction($transaction) {
		return $this->setTransactions ( array_diff ( $this->getTransactions (), array (
				$transaction 
		) ) );
	}
	...
	public function addFailedTransaction($error) {
		if (! $this->getFailedTransactions ()) {
			return $this->setFailedTransactions ( array (
					$error 
			) );
		} else {
			return $this->setFailedTransactions ( array_merge ( $this->getFailedTransactions (), array (
					$error 
			) ) );
		}
	}
	...
	public function removeFailedTransaction($error) {
		return $this->setFailedTransactions ( array_diff ( $this->getFailedTransactions (), array (
				$error 
		) ) );
	}
	...
	/**
	 * Create and process a payment by passing a payment object that includes the intent, payer, and transactions in the body of the request JSON.
	 * For PayPal payments, include redirect URLs in the payment object.
	 *
	 ...
	 * In addition, pass a patch_request_object in the body of the request JSON that specifies the operation to perform, path of the target location, and new value to apply. Please note that it is not possible to use patch after execute has been called.
	 *
	...
	/**
	 * Execute (complete) a PayPal payment that has been approved by the payer by passing the payment_id in the request URI.
	 * This request only works after a buyer has approved the payment using the provided PayPal approval URL. Optionally update transaction information when executing the payment by passing in one or more transactions.
	 *
	...
	/**
	 * List payments in any state (created, approved, failed, etc.).
	 * Payments returned are the payments made to the merchant issuing the request.
	 *
	 * @param array $params        	
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
