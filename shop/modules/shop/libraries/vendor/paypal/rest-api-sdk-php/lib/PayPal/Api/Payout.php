...
class Payout extends PayPalResourceModel {
	 ...
	 * The sender_batch_header contains payout parameters that describe the handling of a batch resource while the items array conatins payout items.
	 *
	 * @param array $params        	
	...
	public function create($params = array(), $apiContext = null, $restCall = null) {
		...
		$json = self::executeCall ( "/v1/payments/payouts" . "?" . http_build_query ( array_intersect_key ( $params, $allowedParams ) ), "POST", $payLoad, null, $apiContext, $restCall );
		$ret = new PayoutBatch ();
		$ret->fromJson ( $json );
		return $ret;
	}
	 ...
	 * You can issue this call multiple times to get the current status.
	 *
