...
class Invoice extends PayPalResourceModel {
	 ...
	 * If left blank will be auto-incremented from the last number. 25 characters max.
	 *
	 * @param string $number        	
	 *
	 * @return $this
	...
	/**
	 * Email address of invoice recipient (required) and optional billing information.
	 * (Note: We currently only allow one recipient).
	 *
	...
	public function addBillingInfo($billingInfo) {
		if (! $this->getBillingInfo ()) {
			return $this->setBillingInfo ( array (
					$billingInfo 
			) );
		} else {
			return $this->setBillingInfo ( array_merge ( $this->getBillingInfo (), array (
					$billingInfo 
			) ) );
		}
	}
	...
	public function removeBillingInfo($billingInfo) {
		return $this->setBillingInfo ( array_diff ( $this->getBillingInfo (), array (
				$billingInfo 
		) ) );
	}
	...
	public function addItem($invoiceItem) {
		if (! $this->getItems ()) {
			...
			return $this->setItems ( array_merge ( $this->getItems (), array (
					$invoiceItem 
			) ) );
		}
	}
	...
	public function removeItem($invoiceItem) {
		return $this->setItems ( array_diff ( $this->getItems (), array (
				$invoiceItem 
		) ) );
	}
	 ...
	 * Date format yyyy-MM-dd z, as defined in [ISO8601](http://tools.ietf.org/html/rfc3339#section-5.6).
	 *
	...
	/**
	 * Optional field to pass payment deadline for the invoice.
	 * Either term_type or due_date can be passed, but not both.
	 *
	 ...
	 * If a label is included then the amount cannot be empty.
	 *
	 ...
	 * If false (the default), the tax is calculated before a discount. If true, the tax is calculated after a discount.
	 *
	...
	/**
	 * A flag indicating whether the unit price includes tax.
	 * Default is false
	 *
	...
	public function addPaymentDetail($paymentDetail) {
		if (! $this->getPayments ()) {
			return $this->setPayments ( array (
					$paymentDetail 
			) );
		} else {
			return $this->setPayments ( array_merge ( $this->getPayments (), array (
					$paymentDetail 
			) ) );
		}
	}
	...
	public function removePaymentDetail($paymentDetail) {
		return $this->setPayments ( array_diff ( $this->getPayments (), array (
				$paymentDetail 
		) ) );
	}
	...
	public function addRefundDetail($refundDetail) {
		if (! $this->getRefunds ()) {
			return $this->setRefunds ( array (
					$refundDetail 
			) );
		} else {
			return $this->setRefunds ( array_merge ( $this->getRefunds (), array (
					$refundDetail 
			) ) );
		}
	}
	...
	public function removeRefundDetail($refundDetail) {
		return $this->setRefunds ( array_diff ( $this->getRefunds (), array (
				$refundDetail 
		) ) );
	}
	...
	/**
	 * Send a specific invoice to its intended recipient by passing the invoice ID to the request URI.
	 * Optionally, you can specify whether to send the merchant an invoice update notification by using the notify_merchant query parameter. By default, notify_merchant is true.
	 *
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
	 * @param PayPalRestCall $restCall
	...
	public function send($apiContext = null, $restCall = null) {
		...
		$payLoad = "";
		self::executeCall ( "/v1/invoicing/invoices/{$this->getId()}/send", "POST", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	
	/**
	 * Send a reminder about a specific invoice to its intended recipient by providing the ID of the invoice in the request URI.
	 * In addition, pass a notification object that specifies the subject of the reminder and other details in the request JSON.
	 *
	...
	/**
	 * Mark the status of an invoice as paid by passing the invoice ID to the request URI.
	 * In addition, pass a payment detail object that specifies the payment method and other details in the request JSON.
	 *
	...
	/**
	 * Mark the status of an invoice as refunded by passing the invoice ID to the request URI.
	 * In addition, pass a refund-detail object that specifies the type of refund and other details in the request JSON.
	 *
	...
	public function recordRefund($refundDetail, $apiContext = null, $restCall = null) {
		...
		self::executeCall ( "/v1/invoicing/invoices/{$this->getId()}/record-refund", "POST", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	...
	/**
	 * Fully update an invoice by passing the invoice ID to the request URI.
	 * In addition, pass a complete invoice object in the request JSON. Partial updates are not supported.
	 *
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
	 * @param PayPalRestCall $restCall
	...
	public function delete($apiContext = null, $restCall = null) {
		ArgumentValidator::validate ( $this->getId (), "Id" );
		$payLoad = "";
		self::executeCall ( "/v1/invoicing/invoices/{$this->getId()}", "DELETE", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	
	/**
	 * Generate a QR code for an invoice by passing the invoice ID to the request URI.
	 * The request generates a QR code that is 500 pixels in width and height. You can change the dimensions of the returned code by specifying optional query parameters.
	 *
	 * @param array $params        	
	 * @param string $invoiceId        	
	 * @param ApiContext $apiContext
