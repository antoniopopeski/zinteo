...
class Plan extends PayPalResourceModel {
	 ...
	 * Allowed values: `CREATED`, `ACTIVE`, `INACTIVE`, and `DELETED`.
	 *
	 * @param string $state        	
	 *
	 * @return $this
	 ...
	 * Format YYYY-MM-DDTimeTimezone, as defined in [ISO8601](http://tools.ietf.org/html/rfc3339#section-5.6).
	 *
	...
	public function addPaymentDefinition($paymentDefinition) {
		if (! $this->getPaymentDefinitions ()) {
			return $this->setPaymentDefinitions ( array (
					$paymentDefinition 
			) );
		} else {
			return $this->setPaymentDefinitions ( array_merge ( $this->getPaymentDefinitions (), array (
					$paymentDefinition 
			) ) );
		}
	}
	...
	public function removePaymentDefinition($paymentDefinition) {
		return $this->setPaymentDefinitions ( array_diff ( $this->getPaymentDefinitions (), array (
				$paymentDefinition 
		) ) );
	}
	...
	public function addTerm($terms) {
		if (! $this->getTerms ()) {
			return $this->setTerms ( array (
					$terms 
			) );
		} else {
			return $this->setTerms ( array_merge ( $this->getTerms (), array (
					$terms 
			) ) );
		}
	}
	...
	public function removeTerm($terms) {
		return $this->setTerms ( array_diff ( $this->getTerms (), array (
				$terms 
		) ) );
	}
	 ...
	 * In addition, pass a patch object in the request JSON that specifies the operation to perform, field to update, and new value for each update.
	 *
	...
	public function update($patchRequest, $apiContext = null, $restCall = null) {
		...
		self::executeCall ( "/v1/payments/billing-plans/{$this->getId()}", "PATCH", $payLoad, null, $apiContext, $restCall );
		return true;
	}
