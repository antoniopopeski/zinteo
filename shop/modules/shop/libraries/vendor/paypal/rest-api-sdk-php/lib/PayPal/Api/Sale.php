...
class Sale extends PayPalResourceModel {
	...
	/**
	 * Indicates the credit status of fund to the recipient.
	 * It will be returned only when payment status is 'completed'
	...
	/**
	 * Retrieve details about a sale transaction by passing the sale_id in the request URI.
	 * This request returns only the sales that were created via the REST API.
	 *
	 * @param string $saleId        	
	 ...
	 * In addition, include an empty JSON payload in the request body for a full refund. For a partial refund, include an amount object in the request body.
	 *
	 * @param Refund $refund        	
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
	...
	public function refund($refund, $apiContext = null, $restCall = null) {
		...
		$json = self::executeCall ( "/v1/payments/sale/{$this->getId()}/refund", "POST", $payLoad, null, $apiContext, $restCall );
		$ret = new Refund ();
		$ret->fromJson ( $json );
		return $ret;
	}
