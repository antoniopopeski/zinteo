...
class WebhookEvent extends PayPalResourceModel {
	 ...
	 * E.g. A successful payment authorization was created for $$
	 *
	...
	/**
	 * Validates Received Event from Webhook, and returns the webhook event object.
	 * Because security verifications by verifying certificate chain is not enabled in PHP yet,
	 ...
	 * Can be used to retrieve the payload for an event.
	 *
	...
	public static function get($eventId, $apiContext = null, $restCall = null) {
		...
		$payLoad = "";
		$json = self::executeCall ( "/v1/notifications/webhooks-events/$eventId", "GET", $payLoad, null, $apiContext, $restCall );
	...
	public function resend($apiContext = null, $restCall = null) {
		...
		$payLoad = "";
		$json = self::executeCall ( "/v1/notifications/webhooks-events/{$this->getId()}/resend", "POST", $payLoad, null, $apiContext, $restCall );
		$this->fromJson ( $json );
		return $this;
	}
	 ...
	 * The developers can use it to see list of past webhooks events.
	 *
	 * @param array $params        	
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
