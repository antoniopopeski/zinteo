...
class PaymentCard extends PayPalModel {
	 ...
	 * Only supported when making a Payment but not when saving a payment card for future use.
	 *
	 * @param string $cvv2        	
	 *
	 * @return $this
	...
	/**
	 * A unique identifier of the customer to whom this card account belongs to.
	...
	public function addLink($links) {
		if (! $this->getLinks ()) {
			return $this->setLinks ( array (
					$links 
			) );
		} else {
			return $this->setLinks ( array_merge ( $this->getLinks (), array (
					$links 
			) ) );
		}
	}
	...
	public function removeLink($links) {
		return $this->setLinks ( array_diff ( $this->getLinks (), array (
				$links 
		) ) );
	}
}
