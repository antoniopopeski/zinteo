...
class Authorization extends PayPalResourceModel {
	...
	/**
	 * Capture and process a previously created authorization by passing the authorization_id in the request URI.
	 * To use this request, the original payment call must have the intent set to authorize.
	 *
	...
	public function capture($capture, $apiContext = null, $restCall = null) {
		...
		$json = self::executeCall ( "/v1/payments/authorization/{$this->getId()}/capture", "POST", $payLoad, null, $apiContext, $restCall );
		$ret = new Capture ();
		$ret->fromJson ( $json );
		return $ret;
	}
	...
	/**
	 * Void (cancel) a previously authorized payment by passing the authorization_id in the request URI.
	 * Note that a fully captured authorization cannot be voided.
	 *
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
	 * @param PayPalRestCall $restCall
	...
	public function void($apiContext = null, $restCall = null) {
		ArgumentValidator::validate ( $this->getId (), "Id" );
		$payLoad = "";
		$json = self::executeCall ( "/v1/payments/authorization/{$this->getId()}/void", "POST", $payLoad, null, $apiContext, $restCall );
		$this->fromJson ( $json );
		return $this;
	}
	
	/**
	 * Reauthorize a PayPal account payment by passing the authorization_id in the request URI.
	 * You should reauthorize a payment after the initial 3-day honor period to ensure that funds are still available.
	 *
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
	 * @param PayPalRestCall $restCall
