...
class Agreement extends PayPalResourceModel {
	...
	public function addOverrideChargeModel($overrideChargeModel) {
		if (! $this->getOverrideChargeModels ()) {
			return $this->setOverrideChargeModels ( array (
					$overrideChargeModel 
			) );
		} else {
			return $this->setOverrideChargeModels ( array_merge ( $this->getOverrideChargeModels (), array (
					$overrideChargeModel 
			) ) );
		}
	}
	...
	public function removeOverrideChargeModel($overrideChargeModel) {
		return $this->setOverrideChargeModels ( array_diff ( $this->getOverrideChargeModels (), array (
				$overrideChargeModel 
		) ) );
	}
	...
	public function create($apiContext = null, $restCall = null) {
		$payLoad = $this->toJSON ();
		$json = self::executeCall ( "/v1/payments/billing-agreements/", "POST", $payLoad, null, $apiContext, $restCall );
		$this->fromJson ( $json );
		return $this;
	}
	...
	public function execute($paymentToken, $apiContext = null, $restCall = null) {
		...
		$payLoad = "";
		$json = self::executeCall ( "/v1/payments/billing-agreements/$paymentToken/agreement-execute", "POST", $payLoad, null, $apiContext, $restCall );
		$this->fromJson ( $json );
		return $this;
	}
	...
	public function update($patchRequest, $apiContext = null, $restCall = null) {
		...
		self::executeCall ( "/v1/payments/billing-agreements/{$this->getId()}", "PATCH", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	...
	public function suspend($agreementStateDescriptor, $apiContext = null, $restCall = null) {
		...
		self::executeCall ( "/v1/payments/billing-agreements/{$this->getId()}/suspend", "POST", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	
	/**
	 * Reactivate a suspended billing agreement by passing the ID of the agreement to the appropriate URI.
	 * In addition, pass an agreement_state_descriptor object in the request JSON that includes a note about the reason for changing the state of the agreement and the amount and currency for the agreement.
	 *
	 * @param AgreementStateDescriptor $agreementStateDescriptor        	
	 * @param ApiContext $apiContext
	 *        	is the APIContext for this call. It can be used to pass dynamic configuration and credentials.
	...
	public function reActivate($agreementStateDescriptor, $apiContext = null, $restCall = null) {
		...
		ArgumentValidator::validate ( $agreementStateDescriptor, 'agreementStateDescriptor' );
		$payLoad = $agreementStateDescriptor->toJSON ();
		self::executeCall ( "/v1/payments/billing-agreements/{$this->getId()}/re-activate", "POST", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	...
	public function cancel($agreementStateDescriptor, $apiContext = null, $restCall = null) {
		...
		ArgumentValidator::validate ( $agreementStateDescriptor, 'agreementStateDescriptor' );
		$payLoad = $agreementStateDescriptor->toJSON ();
		self::executeCall ( "/v1/payments/billing-agreements/{$this->getId()}/cancel", "POST", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	...
	public function billBalance($agreementStateDescriptor, $apiContext = null, $restCall = null) {
		...
		ArgumentValidator::validate ( $agreementStateDescriptor, 'agreementStateDescriptor' );
		$payLoad = $agreementStateDescriptor->toJSON ();
		self::executeCall ( "/v1/payments/billing-agreements/{$this->getId()}/bill-balance", "POST", $payLoad, null, $apiContext, $restCall );
		return true;
	}
	
	/**
	 * Set the balance for an agreement by passing the ID of the agreement to the request URI.
	 * In addition, pass a common_currency object in the request JSON that specifies the currency type and value of the balance.
	 *
	...
	public function setBalance($currency, $apiContext = null, $restCall = null) {
		...
		self::executeCall ( "/v1/payments/billing-agreements/{$this->getId()}/set-balance", "POST", $payLoad, null, $apiContext, $restCall );
		return true;
	}
